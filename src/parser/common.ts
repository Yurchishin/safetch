...
export const pathPatternSegmentValidCharactersRegex = /^[\w-]+$/
...
export abstract class PathPatternSegment<S extends string, P extends string | string[] | null> {
  abstract paramName: string | undefined
  abstract paramSchema: P extends null ? null : Record<string, zod.ZodDefault<zod.ZodType<P>> | zod.ZodType<P>>
  ...
  abstract validatePathPatternSegments(restPathPatternSegments: readonly string[]): void
  ...
  abstract createPathSegment(parameters: Readonly<Record<string, string | readonly string[]>>): string
}

export abstract class NonParamPathPatternSegment extends PathPatternSegment<string, null> {
  override paramName = undefined
  override paramSchema = null

  override createPathSegment(_: Readonly<Record<string, string | readonly string[]>>): string {
    return `${this.segment}`
  }
}

export abstract class ParamPathPatternSegment<S extends string, P extends string | string[]> extends PathPatternSegment<
  S,
  P
> {
...
> extends ParamPathPatternSegment<S, P> {
  ...
  override validatePathPatternSegments(restPathPatternSegments: readonly string[]): void {
    if (!this.matchPathPattern(restPathPatternSegments[0]))
      throw new ValidatePathPatternError(`Invalid symbols in segment "${this.segment}"`)

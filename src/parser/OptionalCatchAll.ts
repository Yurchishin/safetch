...
export const pathPatternOptionalCatchAllSegmentRegex = /^\[{2}\.{3}(?<param>[\dA-Za-z]+)]{2}$/

class OptionalCatchAllPathPatternSegment extends CatchParamPathPatternSegment<
  ValidOptionalCatchAllPathPatternSegment,
  string[]
> {
  static is(segment: string): segment is ValidOptionalCatchAllPathPatternSegment {
    return pathPatternOptionalCatchAllSegmentRegex.test(segment)
  }

  override paramName: string
  override paramSchema: Record<string, zod.ZodDefault<zod.ZodArray<zod.ZodString>>>

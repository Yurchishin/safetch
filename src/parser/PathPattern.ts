...
class PathPattern<Pattern extends string> {
  ...
  constructor(pattern: ValidPathPattern<Pattern>) {
    if (pattern.startsWith('/')) throw new ValidatePathPatternError('Path pattern must not start with "/"')
    if (pattern.endsWith('/')) throw new ValidatePathPatternError('Path pattern must not end with "/"')
    if (pattern.includes('//')) throw new ValidatePathPatternError('Path pattern must not contain "//"')

    const patternStringSegments = pattern.split('/').filter(segment => segment !== '')

    const segments =
      pattern === '/' ? [] : patternStringSegments.map(patternStringSegment => this.createSegment(patternStringSegment))
    ...
    this.paramsSchema = Object.keys(schemaObject).length === 0 ? zod.null() : zod.object(schemaObject)
  }

  validatePath(path: string): void {
    if (this.pattern === '/' && path === '/') return

    if (path.startsWith('/')) throw new ValidatePathError('Path must start with "/"')
    if (path.endsWith('/')) throw new ValidatePathError('Path must not end with "/"')
    if (path.includes('//')) throw new ValidatePathError('Path must not contain "//"')
  ...
  createPath(params: GetParamsFromPathPattern<Pattern>): string {
    ...
    const path = `/${this.segments
      .map(segment => segment.createPathSegment((params as GetParamsFromPathPattern<Pattern> | null) ?? {}))
      .join('/')}`
  ...
  private createSegment(segment: string): PathPatternSegment {
    if (OptionalCatchAllPathPatternSegment.is(segment)) return new OptionalCatchAllPathPatternSegment(segment)
    if (CatchAllPathPatternSegment.is(segment)) return new CatchAllPathPatternSegment(segment)
    if (SingleParamPathPatternSegment.is(segment)) return new SingleParamPathPatternSegment(segment)
    if (StringPathPatternSegment.is(segment)) return new StringPathPatternSegment(segment)

    throw new ValidatePathPatternError(
      `Invalid symbols in segment ${segment as string} at path pattern ${this.pattern}`,
    )
  }

  private validateInternal(segments: readonly PathPatternSegment[], patternStringSegments: readonly string[]): void {
  ...
  private validatePathInternal(segments: readonly PathPatternSegment[], pathSegments: readonly string[]): void {
    ...
    const restPathSegmentsTail = pathSegments.slice(ownPathSegments.length)
    const restSegmentsTail = segments.slice(1)

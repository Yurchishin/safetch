...
describe('SingleParam', () => {
  ...
  it('should throw an error creating instance with an invalid path pattern segment', () => {
    ...
    expect(() => new SingleParam('[...testParam]' as unknown)).toThrow(
      new ValidatePathPatternError('Invalid path pattern segment "[...testParam]"'),
    )
  })
  ...
  it('should correctly validate valid path pattern segments', () => {
    ...
    expect(() => segment.validatePathPatternSegments(['[testParam2]'])).toThrow(
      new ValidatePathPatternError('Invalid symbols in segment "[testParam]" at path pattern'),
    )
  })
  ...
  it('should correctly pick path segments', () => {
    ...
    expect(segment.pickPathSegments(['testParam', 'testParam2'])).toEqual(['testParam'])
  ...
  it('should correctly validate path segments', () => {
    ...
    expect(() => segment.validatePathSegments(['testParam', 'testParam2'])).not.toThrow()
    ...
    expect(() => segment.validatePathSegments([])).toThrow(
      new ValidatePathError('Single segment must have at least one path segment'),
    )
    expect(() => segment.validatePathSegments(['#'])).toThrow(
  ...
  it('should correctly create path segment', () => {
    ...
    expect(segment.createPathSegment({ testParam: 'testParam' })).toBe('testParam')
    expect(segment.createPathSegment({ testParam: 'testParam2' })).toBe('testParam2')
  })
})
